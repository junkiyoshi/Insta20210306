...
void ofApp::update() {
	...
	for (int x = -300; x <= 300; x += 600) {
		...
		auto noise_value = ofNoise(abs(x), 0, ofGetFrameNum() * 0.005);
		rotation = glm::rotate(glm::mat4(), ofMap(noise_value, 0, 1, -180, 180) * (float)DEG_TO_RAD, glm::vec3(0, 1, 0));

		this->setRingToMesh(this->face, this->frame, glm::vec3(x, 0, 0), 0, 50, 10, rotation);
		for (auto radius = 60; radius <= 190; radius += 20) {

			glm::highp_mat4 rotation;
			auto noise_value = ofNoise(abs(x), 0, (ofGetFrameNum() + radius) * 0.005);
			rotation = glm::rotate(glm::mat4(), ofMap(noise_value, 0, 1, -180, 180) * (float)DEG_TO_RAD, glm::vec3(0, 1, 0));

			this->setRingToMesh(this->face, this->frame, glm::vec3(x, 0, 0), radius, 20, 10, rotation);
		}
	}
}
